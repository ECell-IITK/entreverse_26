...
export default function StepTeam({ form, setForm, competition, onNext, onBack }: StepTeamProps) {
  const [errors, setErrors] = useState<{ teamName?: string }>({})

  const validate = () => {
    const e: typeof errors = {}
    if (!form.teamName.trim() || form.teamName.trim().length < 3) e.teamName = 'Team name must be at least 3 characters'
    if (form.teamName.trim().length > 150) e.teamName = 'Team name must be under 150 characters'
    setErrors(e)
      ...
      <div className="flex flex-col gap-5">
        <Field label="Team Name" required error={errors.teamName}>
          <Input
            ...
            value={form.teamName}
            onChange={e => { setForm(f => ({ ...f, teamName: e.target.value })); setErrors({}) }}
