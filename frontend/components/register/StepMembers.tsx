...
export default function StepMembers({
  ...
  const [errors, setErrors] = useState<Array<Partial<Record<keyof Member, string>>>>([])

  const addMember = () => {
    if (form.members.length >= competition.max_team_size) return
    setForm(f => ({ ...f, members: [...f.members, { name: '', roll_no: '', email: '', phone: '', is_leader: false }] }))
  }

  const removeMember = (i: number) => {
    setForm(f => {
      const updated = f.members.filter((_, idx) => idx !== i)
      if (!updated.some(m => m.is_leader) && updated.length > 0) updated[0].is_leader = true
  ...
  const updateMember = (i: number, field: keyof Member, value: string | boolean) => {
    ...
    setErrors(e => { const copy = [...e]; if (copy[i]) delete copy[i][field as keyof Member]; return copy })
  }

  const setLeader = (i: number) => {
    setForm(f => ({
      ...f,
  ...
  const validate = (): boolean => {
    ...
    const errs: Array<Partial<Record<keyof Member, string>>> = form.members.map(m => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="font-heading text-2xl font-bold sm:text-3xl">Add team members</h2>
        <p className="mt-1.5 text-sm text-muted-foreground">
          Team: <span className="font-semibold text-foreground">{form.teamName}</span>
          {' · '}
      ...
      <div className="flex items-center justify-between rounded-xl border border-white/[0.07] bg-white/[0.03] px-4 py-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-medium">{form.members.length}</span>
      ...
      {atMin && form.members.length > 1 && (
        <p className="text-center text-xs text-muted-foreground">
          Minimum team size is {competition.min_team_size}. Remove the last member to go below.
        </p>
      )}
      ...
      <div className="flex items-center justify-between gap-4">
        <button type="button" onClick={onBack}
          disabled={submitting}
          className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-medium text-muted-foreground transition-all hover:border-white/20 hover:text-foreground disabled:opacity-40">
        ...
        <button type="button" onClick={handleSubmit}
          ...
          {submitting ? <><Loader2 className="h-4 w-4 animate-spin" /> Submitting…</> : <>Register <Sparkles className="h-4 w-4" /></>}
        </button>
      </div>
    </div>
  )
