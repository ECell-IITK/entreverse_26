...
import { ArrowRight, CheckCircle2, Loader2, AlertCircle, Users, Trophy } from 'lucide-react'
...
export default function StepCompetition({
  ...
  setForm,
  onNext,
}: StepCompetitionProps) {
  const [codeErr, setCodeErr] = useState('')
  const selected = competitions.find(c => c.id === form.competitionId)

  const validate = () => {
    if (!form.competitionId) { setCodeErr(''); return false }
    if (!form.registrationCode.trim()) { setCodeErr('Registration code is required'); return false }
    setCodeErr('')
    return true
  }
    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="mt-1.5 text-sm text-muted-foreground">
          Select a competition and enter the registration code provided by E-Cell.
        </p>
      </div>
          ...
          {competitions.map((c) => {
            ...
            return (
              <button
                key={c.id}
                type="button"
                onClick={() => setForm(f => ({ ...f, competitionId: c.id, registrationCode: '' }))}
                className={`
      ...
      <AnimatePresence>
        {selected && (
          ...
          >
            <Field label="Registration Code" required error={codeErr}>
              <Input
                type="password"
                placeholder="Enter a code for your identity in E-Cell"
                value={form.registrationCode}
                onChange={e => { setForm(f => ({ ...f, registrationCode: e.target.value })); setCodeErr('') }}
                error={codeErr}
                autoComplete="off"
              />
              <p className="text-[11px] text-muted-foreground/60">
                The registration code is shared via official E-Cell channels for {selected.name}.
              </p>
            </Field>
          </motion.div>
        )}
      ...
      <button
        type="button"
        onClick={handleNext}
        disabled={!form.competitionId || !form.registrationCode.trim()}
