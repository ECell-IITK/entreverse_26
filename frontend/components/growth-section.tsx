...
// Stage tones flow left (deep-blue) → right (electric-cyan),
// mirroring the reference image stream direction.
const STAGES = [
  {
    icon: Lightbulb,
    title: 'Spark',
    desc: 'A raw idea takes shape inside the Innovation Core.',
    tone: 'deep',        // --violet-core #1e3fff
  },
  {
    icon: Users,
    title: 'Collaborate',
    desc: 'Founders, mentors, and investors connect into networks.',
    tone: 'mid',         // --violet-bright #3d6bff
  },
  {
    icon: Rocket,
    title: 'Launch',
    desc: 'Prototypes break orbit and become real ventures.',
    tone: 'electric',    // --magenta-hot #5ec8ff
  },
  {
    icon: Building2,
    title: 'Impact',
    desc: 'Ideas mature into companies that change the world.',
    tone: 'cyan',        // --blue-bright #4fd8ff
  },
]

// Tone → Tailwind classes — all deep-blue family now
const TONE = {
  deep: {
    iconBg:   'bg-[#1e3fff]/20',
  ...
  return (
    <section id="about" className="relative mx-auto max-w-6xl px-4 py-28">
      ...
      >
        ...
        {/* ── "Continuum of Innovation" theme paragraph ──────────────────
            Bright text — no gray. Accent span uses electric-blue highlight.
            ─────────────────────────────────────────────────────────────── */}
        <motion.p
          variants={fade}
          custom={3}
      ...
      <div className="relative mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {/* Connecting line — deep-blue → electric-cyan gradient */}
        ...
        {STAGES.map((s, i) => {
          const t = TONE[s.tone as keyof typeof TONE]
          return (
            <motion.div
              key={s.title}
              variants={fade}
