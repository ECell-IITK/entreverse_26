...
/**
 * SpaceBackground — "Continuum of Innovation"
 *
 * A close recreation of the reference composition:
 *  - a braided violet -> warm-white -> electric-blue wave that crosses
 *    low on the left third of the screen and sweeps to the right edge
 *  - two bright diagonal light streaks tearing off toward the top-left
 *    and (brighter) top-right corners, with a traveling highlight pulse
 *  - small angular "shard" debris scattered near the crossing point
 *  - sparse square dot-grid matrices tucked into the top corners
 *  - a handful of solitary hexagon outlines on the right side
 *  - a soft violet nebula wash behind everything
 *  - a drifting star/dust field with faint connective threads
 *
 * Canvas handles the organic, per-frame stuff (wave, particles, shards,
 * streaks). Framer Motion drives the slower "breathing" glow blobs and
 * the mount-in fade, so motion lives at both the canvas layer and the
 * component layer as requested.
 *
 * NOTE: this file introduces a dependency on `framer-motion`. If it's
 * not already in package.json: npm install framer-motion
 */
...
const VIOLET: [number, number, number] = [30, 63, 255]    // #1e3fff — deep saturated blue
const BLUE: [number, number, number] = [22, 153, 255]
const WARM: [number, number, number] = [94, 200, 255]      // #5ec8ff — electric light-blue highlight
const ICE: [number, number, number] = [190, 230, 255]

const CLEAR_ZONE = { x: 0.52, y: 0.4, rx: 0.34, ry: 0.3 }

function clearZoneFalloff(nx: number, ny: number) {
...
function drawFlowWave(ctx: CanvasRenderingContext2D, width: number, height: number, time: number) {
...
function drawCometStreaks(ctx: CanvasRenderingContext2D, width: number, height: number, time: number) {
  type Streak = { from: [number, number]; to: [number, number]; color: [number, number, number]; width: number; speed: number; phase: number }
...
function drawShards(ctx: CanvasRenderingContext2D, shards: Shard[], time: number) {
...
function drawCornerDots(ctx: CanvasRenderingContext2D, width: number, height: number) {
  const spacing = 20
  const cols = 8
  const rows = 7
  const corners: Array<{ ox: number; oy: number; dir: 1 | -1 }> = [
...
function drawHexAccent(ctx: CanvasRenderingContext2D, cx: number, cy: number, size: number, alpha: number) {
  ctx.beginPath()
  for (let i = 0; i < 6; i += 1) {
...
function drawHexAccents(ctx: CanvasRenderingContext2D, width: number, height: number, time: number) {
...
function drawNebula(ctx: CanvasRenderingContext2D, width: number, height: number, time: number) {
...
export function SpaceBackground() {
  ...
  return (
    <motion.div
      aria-hidden="true"
      /* bg-void lifted to #080d1a */
      className="pointer-events-none fixed inset-0 -z-10 overflow-hidden bg-[#080d1a]"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 1.4, ease: 'easeOut' }}
    >
      {/* Vignette: deepest corners pull to --bg-void-deep #050810 */}
      <div className="absolute inset-0 bg-[radial-gradient(140%_120%_at_50%_10%,#0d1635_0%,#080d1a_55%,#050810_100%)]" />

      {/* Breathing ambient glow blobs:
          Left  = violet-core  #1e3fff  (deep blue, left stream)
          Right = blue-core    #3b6bff  (mid-blue, right stream)
          Centre = dark hollow to keep reading area clear */}
      <motion.div
        /* deep-blue glow, bottom-left — stronger opacity to actually emit light */
      ...
      />
      <motion.div
        /* content-area clearing hollow — dark nebula-haze centre */
      ...
      {/* Edge vignette — pulls bg-void-deep at corners */}
